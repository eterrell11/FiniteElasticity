...
namespace NonlinearElasticity
{
	...
	public:
...

		{
			...
			values[0] = - 1./std::sqrt(mu) * M_PI * M_PI * a * std::sin(M_PI * present_time) * p[1];
			values[1] = 0;
		}
		virtual void
					...
					for (const unsigned int i : fe_values.dof_indices())
					{
						...
						for (const unsigned int j : fe_values.dof_indices())
						{
							cell_mass_matrix(i, j) += (scalar_product(Grad_u_i, trapezoid_toggle * (HH_tilde)*fe_values[Pressure].value(j, q)) - // Kup
													   midpoint_toggle * dt * N_p_i * scalar_product(HH, fe_values[Velocity].gradient(j, q))) *
													  fe_values.JxW(q);
							cell_preconditioner_matrix(i, j) += (1. / kappa * N_p_i * fe_values[Pressure].value(j, q) +
																 dt * 0.5 * (HH_tilde)*Grad_p_i * (HH * fe_values[Pressure].gradient(j, q))) *
																fe_values.JxW(q);
						}
