...
namespace NonlinearElasticity
{
	...
	namespace Parameters
	{
		...
		struct AllParameters : public Materials,
							   public Time,
							   public Numerical,
							   public Simulation
		{
	...
	SchurComplement<PreconditionerType>::SchurComplement(
		...
		const LA::MPI::BlockVector &exemplar,
		const double &kappa)
		: system_matrix(&system_matrix), A_inverse(&A_inverse), exemplar(&exemplar), kappa(kappa), tmp1(exemplar.block(0)), tmp2(exemplar.block(0)), tmp3(exemplar.block(1))
	{
	}
	...
	public:
		...
		{
			values[0] = - 2. * a * M_PI * M_PI * p[0] * (3 * a + a * std::cos(2. * M_PI *present_time) - 4 * std::sin(M_PI * present_time)) / 
						((-2. + a * std::sin(M_PI * present_time)) * (-2. + a * std::sin(M_PI * present_time)) * (-2. + a * std::sin(M_PI * present_time)));
			values[1] = 2. * a * M_PI * M_PI * p[1] * (3 * a + a * std::cos(2. * M_PI *present_time) + 4 * std::sin(M_PI * present_time)) / 
						((2. + a * std::sin(M_PI * present_time)) * (2. + a * std::sin(M_PI * present_time)) * (2. + a * std::sin(M_PI * present_time)));
		}
		virtual void
	...
	template <int dim>
	class FExtPressure : public Function<dim>
	{
	public:
		virtual void rhs_value(const Point<dim> /*&p*/, double &value, double /*&a*/, double /*&present_time*/)
		...
		virtual void
		rhs_value_list(const std::vector<Point<dim>> &points, std::vector<double> &value_list, double &BodyForce, double &present_time)
		{
			const unsigned int n_points = points.size();
			Assert(value_list.size() == n_points, ExcDimensionMismatch(value_list.size(), n_points));
			for (unsigned int p = 0; p < n_points; ++p)
				FExtPressure<dim>::rhs_value(points[p], value_list[p], BodyForce, present_time);
		}
	};
	...
	InitialSolution<dim>::InitialSolution(double &InitialVelocity, double &mu)
		: Function<dim>(dim + 1), velocity(InitialVelocity), mu(mu)
	{
	}
	...
	{
	public:
		VelocitySolution(double& InitialVelocity, double& mu, double& present_time);
		virtual void vector_value(const Point<dim>& p,
			Vector<double>& values) const override;
		virtual void
			vector_value_list(const std::vector<Point<dim>>& points,
	...
	template <int dim>
	VelocitySolution<dim>::VelocitySolution(double& InitialVelocity, double& mu, double& present_time)
		: Function<dim>(dim + 1)
		, a(InitialVelocity)
		, mu(mu)
		, time(present_time)
	...
	template <int dim>
	void
		VelocitySolution<dim>::vector_value(const Point<dim>& p,
			Vector<double>& values) const
	{
		Assert(values.size() == (dim + 1), ExcDimensionMismatch(values.size(), dim + 1));

		values[0] = 4 * a * M_PI * p[0] * std::cos(M_PI * time) /
		((-2. + a * std::sin(M_PI * time)) * (-2. + a * std::sin(M_PI * time)));

		values[1] = - 4 * a * M_PI * p[1] * std::cos(M_PI * time) /
		((2. + a * std::sin(M_PI * time)) * (2. + a * std::sin(M_PI * time)));
	...
	template <int dim>
	void VelocitySolution<dim>::vector_value_list(
		const std::vector<Point<dim>>& points,
		std::vector<Vector<double>>& value_list) const
	{
		const unsigned int n_points = points.size();
		Assert(value_list.size() == n_points, ExcDimensionMismatch(value_list.size(), n_points));
		for (unsigned int p = 0; p < n_points; ++p)
			VelocitySolution<dim>::vector_value(points[p], value_list[p]);
	}
	...
	void
		Solution<dim>::vector_value(const Point<dim>& p,
			Vector<double>& values) const
	{
		values[0] = 2 * a * p[0] * std::sin(M_PI * time) /
		(2. - a * std::sin(M_PI * time));

		values[1] = - 2 * a * p[1] * std::sin(M_PI * time) /
		(2. + a * std::sin(M_PI * time));
	...
	template <int dim>
	class PressureValues : public Function<dim>
	{
	public:
		PressureValues(double &present_time, double &velocity);
		virtual void vector_value(const Point<dim> &p,
								  Vector<double> &values) const override;
		virtual void
		vector_value_list(const std::vector<Point<dim>> &points,
	...
	template <int dim>
	PressureValues<dim>::PressureValues(double &present_time, double &velocity)
		: Function<dim>(dim + 1),
		  time(present_time),
		  a(velocity)
	{
		...
		std::vector<Vector<double>> &value_list) const
	{
		...
		Assert(value_list.size() == n_points, ExcDimensionMismatch(value_list.size(), n_points));
		for (unsigned int p = 0; p < n_points; ++p)
			PressureValues<dim>::vector_value(points[p], value_list[p]);
	}
	...
	{
		Assert(values.size() == (dim + 1), ExcDimensionMismatch(values.size(), dim + 1));
		values[0] = a * M_PI * std::cos(M_PI * time) * p[1];
		values[1] = 0;
	}
	template <int dim>
	...
	void Incompressible<dim>::run()
	{
		...
		for (int ref_step = 0; ref_step < max_it; ++ref_step)
		{
			...
			{
				TimerOutput::Scope timer_section(timer, "Setting up system");
				setup_system();
			}
			if (ref_step == 0)
			{
	...
	void Incompressible<dim>::create_simplex_grid(parallel::shared::Triangulation<2> &triangulation)
	{
		...
		quad_triangulation.refine_global(parameters.n_ref);
	...
	void Incompressible<dim>::setup_system()
	{
		...
		// HOMOGENEOUS CONSTRAINTS
		{
			...
			VectorTools::interpolate_boundary_values(*(mapping_ptr),
				dof_handler,
				0,
				PressureValues<dim>(present_time, parameters.InitialVelocity),
				constraints,
						...
						for (const unsigned int i : fe_values.dof_indices())
						{
							...
							for (const unsigned int j : fe_values.dof_indices())
							{
								cell_mass_matrix(i, j) += (scale * N_u_i * fe_values[Velocity].value(j, q) +
														   N_p_i * fe_values[Pressure].value(j, q)) *
														  fe_values.JxW(q);
							}
						}
					}
	...
	void Incompressible<dim>::assemble_system_SBDF2()
	{
		...
		FExtPressure<dim> rhs_pressure;
		std::vector<double> rhs_pressure_values(n_q_points);
		...
		Tensor<2, dim> Grad_u_i;
		Tensor<1, dim> N_u_i;
		double N_p_i;
		Tensor<1, dim> Grad_p_i;

		{
			...
			for (const auto &cell : dof_handler.active_cell_iterators())
			{
				if (cell->subdomain_id() == this_mpi_process)
				{
					...
					rhs_pressure.rhs_value_list(fe_values.get_quadrature_points(), rhs_pressure_values, parameters.BodyForce, present_time);
					...
					for (const unsigned int q : fe_values.quadrature_point_indices())
					{
						...
						for (const unsigned int i : fe_values.dof_indices())
						{
							Grad_u_i = fe_values[Velocity].gradient(i, q);
							N_u_i = fe_values[Velocity].value(i, q);
							N_p_i = fe_values[Pressure].value(i, q);
							Grad_p_i = fe_values[Pressure].gradient(i, q);
							for (const unsigned int j : fe_values.dof_indices())
							{
								...
								cell_preconditioner_matrix(i, j) += (1. / kappa * N_p_i * fe_values[Pressure].value(j, q) +
																	dt * scale * (HH_tilde) * Grad_p_i * (HH * fe_values[Pressure].gradient(j, q))) *
																	fe_values.JxW(q);
							}
							cell_rhs(i) += (-scale * scalar_product(Grad_u_i, pk1_dev_tilde) +
											rho_0 * scale * N_u_i * rhs_values[q]  +
											N_p_i * w_prime + N_p_i * rhs_pressure_values[q]) *
										fe_values.JxW(q);
						}
					}
					for (const auto &face : cell->face_iterators())
		...
		{
			TimerOutput::Scope timer_section(timer, "Compressing system");
			K.compress(VectorOperation::add);
			...
			if (cell->subdomain_id() == this_mpi_process)
			{
				...
				for (const unsigned int q : fe_values.quadrature_point_indices())
				{
					...
					for (const unsigned int i : fe_values.dof_indices())
					{
						...
						Tensor<1, dim> N_u_i = fe_values[Velocity].value(i, q);
						cell_rhs(i) += 0.5 * (scalar_product(Grad_u_i, pk1_dev + temp_pressure * HH) + rho_0 * N_u_i * rhs_values[q]) * fe_values.JxW(q);
					}
				}
				for (const auto &face : cell->face_iterators())
				{
	...
	void Incompressible<dim>::solve_SBDF2()
	{
		...
		{
			solution.block(0).add(dt, velocity);
		}
