...
namespace Project_attempt
{
	...
	public:
		Hemisphere();
		~Hemisphere();  //Provides a destructor
		void run();
	...
	public:
			...
			double value(const Point<dim>& p, unsigned int component = 0) const override
		{
			(void)component;
			double nu = 0.35;
			double h = (4 + 4 * p[0]); //Transformes x-value into depth
			 //cubic function from experimental data
			double E = -.47 * h * h * h + 4.7 * h * h - 16.3 * h + 22.7;
			E *= 1000;
			double return_value = E / (2 * (1 + nu));
			 //definition of lambda using E and nu
			return return_value;
		}
	};
			...
			Vector<double>& values) const
	{
		Assert(values.size() == dim, ExcDimensionMismatch(values.size(), dim));
		values = 0;
		double iterative_multiplier = 1 -
			(end_time - present_time) /
			(end_time - present_time + present_timestep);
		values(0) = -p[0] * iterative_multiplier;
	}
	template <int dim>
	...
	template <int dim>
	void Hemisphere<dim>::create_coarse_grid()
	{

		const Point<3> center(-1, 0, 0);
		const double inner_radius = 0.75, outer_radius = 1;
		GridGenerator::half_hyper_shell(triangulation, center, inner_radius, outer_radius, 12); //This is the real domain
		...
		for (const auto& cell : triangulation.cell_iterators())
			for (const auto& face : cell->face_iterators())
			{
				...
				const Tensor<1, 3> diff = face_center - center;
				if (std::fabs(diff.norm_square() - (outer_radius)) < 2.5e-2) {
	...
	template <int dim>
	void Hemisphere<dim>::setup_system()
	{
		...
		VectorTools::interpolate_boundary_values(dof_handler, 2, Functions::ZeroFunction<dim>(dim), hanging_node_constraints); //Establishes zero BCs, => must replace
	...
	template <int dim>
	void Hemisphere<dim>::assemble_system()
	{
		system_rhs = 0;
		system_matrix = 0;
		...
		// std::vector<double> E(n_q_points); // This term isn't really needed here, would be nice to display elasticity on mesh
			...
			if (cell->is_locally_owned())
			{
				...
				for (const unsigned int i : fe_values.dof_indices())
				{
					...
					for (const unsigned int j : fe_values.dof_indices())
					{
						...
						{
							double lambda = local_quadrature_points_data[q_point].lambda;
							double mu = local_quadrature_points_data[q_point].mu;
							cell_matrix(i, j) +=
	...
	template<int dim>
	void Hemisphere<dim>::record_initial_coord()
	{
		FEValues<dim> fe_values(fe,
			quadrature_formula,
		...
		for (auto& cell : dof_handler.active_cell_iterators())
			if (cell->is_locally_owned())
			{
				...
				for (const unsigned int q_point : fe_values.quadrature_point_indices()) {
					local_quadrature_points_history[q_point].lambda = lambda_values[q_point];
					local_quadrature_points_history[q_point].mu = mu_values[q_point];
				}
			}
	}
	...
	template<int dim>
	void Hemisphere<dim>::solve_timestep()
	{
	...
	template <int dim>
	unsigned int Hemisphere<dim>::solve()
	{
	...
	template<int dim>
	void Hemisphere<dim>::output_results() const
	{
		...
		Vector<double> normal_strain(triangulation.n_active_cells());
		{
			for (auto& cell : triangulation.active_cell_iterators())
				if (cell->is_locally_owned())
				{
					...
					for (unsigned int q = 0; q < quadrature_formula.size(); ++q) {
						...
						cum_strain = reinterpret_cast<PointHistory<dim>*>(cell->user_pointer())[q].cum_strain;
						e_nm += 1 / (1+cum_strain) - 1;
						//pcout << e_nm << std::endl;
					}
					norm_of_stress(cell->active_cell_index()) = (accumulated_stress / quadrature_formula.size()).norm();
					normal_strain(cell->active_cell_index()) = (e_nm / quadrature_formula.size());
		...
		data_out.add_data_vector(normal_strain, "normal_strain");
	...
	template<int dim>
	void Hemisphere<dim>::do_initial_timestep()
	{
	...
	template <int dim>
	void Hemisphere<dim>::do_timestep()
	{
		present_time += present_timestep;
		++timestep_no;
	...
	template <int dim>
	void Hemisphere<dim>::refine_grid()
	{
	...
	template<int dim>
	void Hemisphere<dim>::setup_quadrature_point_history()
	{
	...
	template<int dim>
	void Hemisphere<dim>::update_quadrature_point_history()
	{
		FEValues<dim> fe_values(fe,
			quadrature_formula,
			update_values | update_gradients | update_quadrature_points);
		...
		for (auto& cell : dof_handler.active_cell_iterators())
			if (cell->is_locally_owned())
			{
				...
				for (unsigned int q = 0; q < quadrature_formula.size(); ++q)
				{
					...
					double lambda = local_quadrature_points_history[q].lambda;
					double mu = local_quadrature_points_history[q].mu;
					const SymmetricTensor<4, dim> stress_strain_tensor =
						get_stress_strain_tensor<dim>(lambda, mu);
					...
					Tensor<2, dim> strain = get_strain(displacement_increment_grads[q]);
					local_quadrature_points_history[q].cum_strain = strain[0][0]/location[0];
...
int main(int argc, char** argv)
{
	try
	{
		...
		Project_attempt::Hemisphere<3> hemisphere;
		hemisphere.run();
	}
