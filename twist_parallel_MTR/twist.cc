...
template <class PreconditionerType>
	...
	public:
		...
		{	
			double ratio = 0.5; //How much of total run time to add forcing
		...
		virtual void
			rhs_vector_value_list(const std::vector<Point<dim>>& points, std::vector<Tensor<1, dim>>& value_list, double& BodyForce, double& present_time, double& end_time)
		{
	...
	void Incompressible<dim>::measure_energy()
	{
		...
		for (const auto& cell : dof_handler.active_cell_iterators())
		{
			...
			for (const unsigned int q : fe_values.quadrature_point_indices())
			{
				...
				total_energy +=  (rho_0 * 0.5 * vn * vn // Kinetic energy
				...
				if (cell->subdomain_id() == this_mpi_process)
				{
					for (const unsigned int i : fe_values.dof_indices())
					{
						cell_energy(i) += fe_values[Pressure].value(i,q) * (rho_0 * 0.5 * vn * vn // Kinetic energy
						+ 0.5 * mu * (std::cbrt(1. / (Jf * Jf)) * scalar_product(FF, FF) - double(dim)) //Deviatoric energy
	...
	void Incompressible<dim>::solve_SBDF2_system()
	{
		...
		PreconditionIdentity preconditioner_S_in;
		...
		SolverControl solver_control_S(2000, 1.0e-14);
