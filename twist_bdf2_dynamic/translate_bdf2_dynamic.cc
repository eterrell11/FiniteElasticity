...
namespace NonlinearElasticity
{
	...
	void Incompressible<dim>::assemble_system_SI()
	{
		...
		for (const auto& cell : dof_handler.active_cell_iterators())
		{
			...
			fe_values[Velocity].get_function_values(solution_increment, sol_vec_velocity);
			...
			for (const auto& face : cell->face_iterators())
			{
				if (face->at_boundary())
				{
					...
					fe_face_values[Velocity].get_function_values(solution_increment, face_sol_vec_velocity);
	...
	void Incompressible<dim>::solve_SI_system()
	{
		...
		auto& v = solution_increment.block(0);
	...
	void Incompressible<dim>::update_motion()
	{
		...
		velocity = solution_increment.block(0);
