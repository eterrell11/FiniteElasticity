...
namespace NonlinearElasticity
{
	...
	namespace ConstitutiveModels
	{
		...
		public:
			...
			Tensor<1, dim>  W_prime_dt_IBP(const int& wvol_form, const Tensor<2, dim>& HH, const Tensor<2, dim>& FF_inv_T, const Tensor<1, dim>& v);
		};
		...
		template<int dim>
		Tensor<1,dim>	WVol<dim>::W_prime_dt_IBP(const int& wvol_form, const Tensor<2, dim>& HH, const Tensor<2, dim>& FF_inv_T, const Tensor<1, dim>& v)
		{
			if (wvol_form == 0) {
				return transpose(HH) * v;
			}
			else if (wvol_form == 1) {
				return transpose(FF_inv_T) * v;
			}
		}
	...
	private:
		...
		BlockVector<double> old_solution_dot;
	...
	void Incompressible<dim>::set_simulation_parameters()
	{
		...
		wvol_form = parameters.WVol_form;

		height = 3;
	}
	template <int dim>
	void Incompressible<dim>::set_timestep_parameters()
	{
		double temp_E = E * 10.;

		double temp_kappa = get_kappa<dim>(temp_E, nu);
		double c_p = sqrt((4. / 3. * mu + temp_kappa) / rho_0);
		double c_nu = sqrt(mu / rho_0);
		double min_K_step = cell_measure / c_p;
		double min_mu_step = cell_measure / c_nu;
		cout << min_mu_step << std::endl;
		double Tau_K = parameters.tau * 0.01 * std::max(min_K_step / 100., std::min(min_K_step, dt));
		double Tau_mu = parameters.tau * 0.5 * std::max(min_mu_step / 100., std::min(min_mu_step, dt));
		...
		height = 6;
		std::cout << "DVMS stabilization parameter : " << Tau_cp << std::endl;
		std::cout << "pressure wave speed is : " << c_p << std::endl;
		std::cout << "min timestep is " << min_K_step << std::endl;
	}
	...
	void Incompressible<dim>::setup_system()
	{
		...
		old_solution_dot.reinit(dofs_per_block);
		...
		old_solution_dot.block(0) = solution.block(0);
		old_u_dot = old_solution_dot.block(0);
			...
			for (const auto& cell : dof_handler.active_cell_iterators())
			{
				...
				for (const unsigned int q : fe_values.quadrature_point_indices())
				{
					...
					for (const unsigned int i : fe_values.dof_indices())
					{
						...
						{
							cell_mass_matrix(i, j) += 1. / kappa / dt * N_p_i * fe_values[Pressure].value(j, q) * fe_values.JxW(q);
					...
					for (const unsigned int i : fe_values.dof_indices())
					{
						...
						for (const unsigned int j : fe_values.dof_indices())
						{
							cell_mass_matrix(i, j) += (rho_0 / dt * N_u_i * fe_values[Velocity].value(j, q) +
								1. / kappa / dt * N_p_i * fe_values[Pressure].value(j, q)) * fe_values.JxW(q);
						}
					}
				}
				cell->get_dof_indices(local_dof_indices);
	...
	void Incompressible<dim>::assemble_system_SI()
	{
		...
		std::vector<Tensor<1, dim>> rhs_face_values(n_face_q_points, Tensor<1, dim>());
		...
		std::vector<Tensor<2, dim>> old_face_displacement_grads(n_face_q_points, Tensor<2, dim>());
		...
		std::vector<Tensor<1, dim>> old_sol_vec_velocity(n_q_points, Tensor<1, dim>());
		std::vector<Tensor<1, dim>> face_sol_vec_velocity(n_face_q_points, Tensor<1, dim>());
		std::vector<Tensor<1, dim>> old_face_sol_vec_velocity(n_face_q_points, Tensor<1, dim>());
		...
		for (const auto& cell : dof_handler.active_cell_iterators())
		{
			...
			fe_values[Velocity].get_function_values(old_solution_dot, old_sol_vec_velocity);
			...
			for (const unsigned int q : fe_values.quadrature_point_indices())
			{
				...
				FF_inv_T_tilde = 2. * HH / Jf - old_HH / old_Jf;
				pk1_dev_tilde = 2. * pk1_dev - old_pk1_dev;

				for (const unsigned int i : fe_values.dof_indices())
				{
					auto Grad_v_i = fe_values[Velocity].gradient(i, q);
					Tensor<1, dim> N_u_i = fe_values[Velocity].value(i, q);
					double N_p_i = fe_values[Pressure].value(i, q);
					...
					for (const unsigned int j : fe_values.dof_indices())
					{
						...
						cell_mass_matrix(i, j) += (scale * scalar_product(Grad_v_i, (HH_tilde)*fe_values[Pressure].value(j, q)) - //Kup
							scale * N_p_i * W_prime_dt +
							1. / kappa / dt * N_p_i * fe_values[Pressure].value(j, q)) * fe_values.JxW(q);
						if (parameters.velocity_order == 1 && parameters.tau != 0) {
							cell_mass_matrix(i, j) += Tau_cp * HH_tilde * Grad_p_i * scale * (-fe_values[Velocity].value(j, q) / dt + scale * HH_tilde * fe_values[Pressure].gradient(j, q)) * fe_values.JxW(q);
						}

					}
					cell_rhs(i) += (-scale * scalar_product(Grad_v_i, pk1_dev_tilde) +
						scale * N_u_i * rhs_values[q] +
						1. / kappa / dt * N_p_i * ((2. - scale) * p_n - (1. - scale) * p_n_minus)) * fe_values.JxW(q);
					if (parameters.velocity_order == 1 && parameters.tau != 0) {
						cell_rhs(i) += Tau_cp * HH_tilde * Grad_p_i * scale * ((-(2. - scale) * v_n + (1. - scale) * v_n_minus) / dt - scale * rhs_values[q]) * fe_values.JxW(q);
					}
				}
			}
			for (const auto& face : cell->face_iterators())
			{
				if (face->at_boundary())
				{
					...
					fe_face_values[Velocity].get_function_gradients(old_solution, old_face_displacement_grads);
					fe_face_values[Velocity].get_function_values(solution_dot, face_sol_vec_velocity);
					fe_face_values[Velocity].get_function_values(old_solution_dot, old_face_sol_vec_velocity);
					...
					right_hand_side.rhs_vector_value_list(fe_face_values.get_quadrature_points(), rhs_face_values, parameters.BodyForce, present_time, mu, kappa);

					for (const unsigned int q : fe_face_values.quadrature_point_indices())
					{
						v_n = face_sol_vec_velocity[q];
						v_n_minus = old_face_sol_vec_velocity[q];
						FF = get_real_FF(face_displacement_grads[q]);
						Jf = get_Jf(FF);
						HH = get_HH(FF, Jf);

						old_FF = get_real_FF(old_face_displacement_grads[q]);
						old_Jf = get_Jf(old_FF);
						old_HH = get_HH(old_FF, old_Jf);
						...
						for (const unsigned int i : fe_face_values.dof_indices())
						{
							...
							if (parameters.velocity_order == 1 && parameters.tau != 0)
							{
								cell_rhs(i) -= scale *Tau_cp * fe_face_values[Pressure].value(i, q) * transpose(HH_tilde) * ((-(2. - scale) * v_n + (1. - scale) * v_n_minus) / dt - scale * rhs_values[q]) * fe_face_values.normal_vector(q) * fe_face_values.JxW(q);
								for (const unsigned int j : fe_face_values.dof_indices())
								{
									cell_mass_matrix(i, j) += scale * Tau_cp * fe_face_values[Pressure].value(i, q) * transpose(HH_tilde) * (fe_face_values[Velocity].value(j, q) / dt - HH_tilde * fe_face_values[Pressure].gradient(j, q)) * fe_face_values.normal_vector(q) * fe_face_values.JxW(q);
								}
							}
						}
	...
	void Incompressible<dim>::assemble_Rp()
	{
		...
		std::vector<Tensor<2, dim>> displacement_grads(n_q_points, Tensor<2, dim>());
		std::vector<Tensor<1, dim>> pressure_grads(n_q_points, Tensor<1, dim>());
		std::vector<Tensor<2, dim>> face_displacement_grads(n_face_q_points, Tensor<2, dim>());
		std::vector<Tensor<1, dim>> sol_vec_velocity(n_q_points, Tensor<1, dim>());
		std::vector<Tensor<1, dim>> old_sol_vec_velocity(n_q_points, Tensor<1, dim>());
		std::vector<Tensor<1, dim>> face_sol_vec_velocity(n_face_q_points, Tensor<1, dim>());
		...
		for (const auto& cell : dof_handler.active_cell_iterators())
		{
			...
			fe_values[Velocity].get_function_gradients(solution, displacement_grads);
			fe_values[Pressure].get_function_gradients(solution, pressure_grads);
			...
			for (const unsigned int q : fe_values.quadrature_point_indices())
			{
				...
				FF = get_real_FF(displacement_grads[q]);
				Jf = determinant(FF);
				FF_inv_T = transpose(invert(FF));
				HH = Jf * FF_inv_T;
				//HH_tilde = 2. * HH - old_HH;
				//pk1_dev_tilde = 2. * pk1_dev - old_pk1_dev;
				w_prime_dt_IBP = wvol.W_prime_dt_IBP(wvol_form, HH, FF_inv_T, v_n);

				//temp_pressure -= pressure_mean;
				for (const unsigned int i : fe_values.dof_indices())
				{
					auto Grad_p_i = fe_values[Pressure].gradient(i, q);
					cell_rhs(i) -= (Grad_p_i * w_prime_dt_IBP) * fe_values.JxW(q);
					if (parameters.tau != 0 && parameters.velocity_order == 1) {
						cell_rhs(i) += Tau_cp * HH * Grad_p_i * (rho_0 * (v_n - v_n_minus)/dt - HH * pressure_grads[q] - rhs_values[q]) * fe_values.JxW(q);
					}
				}
			}
			for (const auto& face : cell->face_iterators())
				...
				if (face->at_boundary())
				{
					...
					for (const unsigned int q : fe_face_values.quadrature_point_indices())
					{
						...
						for (const unsigned int i : fe_face_values.dof_indices())
						{
							...
							cell_rhs(i) += N_p_i * w_prime_dt_IBP * fe_face_values.normal_vector(q) * fe_face_values.JxW(q);
	...
	void Incompressible<dim>::solve_FE()
	{
		...
		solution.block(0) += dt * solution_dot.block(0);
		{
			solve_FEp_system();
		}
		{
		...
		solution *= 0.5;
		solution_dot.block(0) += solution_dot_save.block(0);
		solution_dot.block(0) *= 0.5;
		calculate_error();
	...
	void Incompressible<dim>::solve_SI_system()
	{
		...
		Vector<double> un_pressure(solution.block(1).size());
		if (present_time < dt) {
			un_motion.add(1.0, velocity, 0.0, old_solution_dot.block(0));
			//un_pressure.add(1.0, solution.block(1), 0.0, old_solution.block(1));
		}
		else
		{
			un_motion.add(4. / 3., velocity, -1. / 3., old_solution_dot.block(0));
			//un_pressure.add(4./3., solution.block(1), -1./3., old_solution.block(1));

		}
		old_solution.block(1) = solution.block(1);

		Kuu.vmult_add(Ru, un_motion);
		//Kpp.vmult_add(Rp, un_pressure);
	...
	void Incompressible<dim>::solve_FEv_system()
	{
		...
		auto& v = solution_dot.block(0);
		Vector<double> a = solution_dot.block(0);
		...
		solver_Kvv.solve(Kuu, a, Ru, preconditioner_Kvv);
		v += a;
		constraints.distribute(solution_dot);
		constraints.distribute(solution);
	}
	...
	{
		old_solution_dot.block(0) = velocity;
	...
	void Incompressible<dim>::do_timestep()
	{
		...
		else if (parameters.integrator == 1)
		{
			solve_SI();
		}
